# -*- coding: utf-8 -*-

#import magic
import os
import cv2
import time
import datetime
import multiprocessing
import face_recognition
import json
import base64
from flask import Flask, make_response, Response, request, render_template, redirect
from werkzeug.utils import secure_filename
from flask_cors import CORS

import db_helper

# configuration
DEBUG = True

COUNT_FRAME_WITH_DATA = 50
FRAME_HEIGHT = 120
FRAME_WIDTH = 160
FRAME_FPS = 50
TIME_DETECTION = 0.2 # обновлять время последней детекции каждые N мин
TIME_DETECTION_UNKNOWN = 0.2

def StartWebServer(qI, port=5000):
    app = Flask(__name__)
    # app.config.from_object(__name__)

    # Flask-CORS нужен для отправки cross-origin-запросов (запросы, исходящие из другого протокола, IP-адреса, 
    # имени домена или порта), поэтому необходимо включить общий доступ к ресурсам (CORS).
    CORS(app)

    def eventStream():
        while True:
            result = qI.get()
            yield 'data: %s\n\n' % str(result)

    def getEvents():
        events = db_helper.get_events()
        return json.dumps(events, default=str)

    def getCameras():
        cameras = db_helper.get_cameras()
        return json.dumps(cameras, default=str)
    
    def getFaces():
        faces = db_helper.get_faces(None, False)
        return json.dumps(faces, default=str)

    @app.route("/events_data")
    def events_data():
        return Response(getEvents(), mimetype="text/event-stream")

    @app.route("/events_clear")
    def events_clear():
        db_helper.clear_events()
        return ''

    @app.route("/cameras_data")
    def cameras_data():
        return Response(getCameras(), mimetype="text/event-stream")

    @app.route("/faces_data")
    def faces_data():
        return Response(getFaces(), mimetype="text/event-stream")

    @app.route("/save_face", methods=["POST"])
    def save_face():
        name = request.form['name']
        lastname = request.form['lastname']
        fathername = request.form['fathername']
        photofile = request.files['photo']
        print(request.form)
        print(photofile)

        if photofile and name and lastname:
            time_adding = datetime.datetime.now()
            relative_path = 'static/DataSet/'
            directory = os.getcwd() + '/' + relative_path
            file_name = name + '_' + lastname + '_' + str(time_adding) + '.jpg'
            full_path = directory + file_name
            photo_path = relative_path + file_name
            photofile.save(full_path)
            added_id = db_helper.insert_new_face(False, photo_path, name, lastname, fathername)
            if added_id:
                print('ADDED')
            return redirect("/faces", code=302)
        else:
            return 'ERROR'

    @app.route("/delete_face", methods=["POST"])
    def delete_face(*args, **kwargs):
        data = request.json
        id = data['face_id']
        db_helper.delete_face(id)
        return str(True)

    @app.route("/stream")
    def stream():
        return Response(eventStream(), mimetype="text/event-stream")

    @app.route("/")
    def index():
        return redirect("/events", code=302)

    @app.route("/delete_event", methods=["POST"])
    def delete_event(*args, **kwargs):
        data = request.json
        id = data['event_id']
        db_helper.delete_event(id)
        return str(True)

    @app.route("/events")
    def events():
        return render_template('events.html')

    @app.route("/cameras")
    def cameras():
        return render_template('cameras.html')

    @app.route("/faces")
    def faces():
        return render_template('faces.html')

    app.run("", port)
    app.debug = True

def CheckFace(cap, qI, camera, count_frame):
    i = 0
    while True:
        ret, img = cap.read()
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        faces = detector.detectMultiScale(gray, 1.3, 5)
        for (x, y, w, h) in faces:
            cv2.rectangle(img, (x, y), (x + w, y + h), (255, 0, 0), 2)
        cv2.imshow(camera['name'], img)
        if i % count_frame == 0:
            qI.put({'camera_name': camera['name'], 'camera_id': camera['id'], 'frame': img})
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
        i += 1
    cap.release()
    cv2.destroyAllWindows()


def FullRecognizedFace(qI, qO):
    # faces = db_helper.get_faces(None, False)
    LAST_TIME_DETECTION_UNKNOWN = datetime.datetime.now()

    # Загружаем все знакомые лица
    # known_face_encodings = []
    # known_face_ids = []
    # directory = os.getcwd() + '/'

    # for face in faces:
    #     print(face['photo_path'])
    #     path_to_image = directory + face['photo_path']
    #     image = face_recognition.load_image_file(path_to_image)
    #     encoding = face_recognition.face_encodings(image)
    #     if len(encoding) > 0:
    #         face_encoding = encoding[0]
    #         known_face_encodings.append(face_encoding)
    #         known_face_ids.append(face['face_id'])

    i = 0
    while True:
        data_frame = qI.get()

        # Загружаем все знакомые лица
        faces = db_helper.get_faces(None, False)
        known_face_encodings = []
        known_face_ids = []
        directory = os.getcwd() + '/'

        for face in faces:
            print(face['photo_path'])
            path_to_image = directory + face['photo_path']
            image = face_recognition.load_image_file(path_to_image)
            encoding = face_recognition.face_encodings(image)
            if len(encoding) > 0:
                face_encoding = encoding[0]
                known_face_encodings.append(face_encoding)
                known_face_ids.append(face['face_id'])

        camera_name = data_frame.get('camera_name')
        camera_id = data_frame.get('camera_id')
        frame = data_frame.get('frame')
        small_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25)
        rgb_small_frame = small_frame[:, :, ::-1]
        face_locations = face_recognition.face_locations(rgb_small_frame)
        face_encodings = face_recognition.face_encodings(rgb_small_frame, face_locations)
        for face_encoding in face_encodings:
            matches = face_recognition.compare_faces(known_face_encodings, face_encoding)
            face_id = None

            if True in matches:
                first_match_index = matches.index(True)
                face_id = known_face_ids[first_match_index]
            
            # лицо определено
            if face_id is not None:
                face_info = db_helper.get_faces(face_id)[0]

                try:
                    img_data = open(face_info['photo_path'], 'rb' ).read()
                    img_data = "data:image/png;base64," + base64.b64encode(img_data).decode()
                except:
                    img_data = "data:image/jpeg;base64,"

                time_detection = datetime.datetime.now()
                last_detection = face_info['last_detection']

                if last_detection:
                    delta_time_detection = time_detection - last_detection
                    minutes_passed = delta_time_detection.seconds / 60
                else:
                    minutes_passed = TIME_DETECTION + 1

                print('compare last detec: ' + str(minutes_passed))

                if minutes_passed > TIME_DETECTION:
                    # обновить время последней детекции
                    db_helper.update_last_detection(face_info['face_id'], time_detection)

                    face_info['datetime'] = time_detection
                    face_info['camera'] = camera_name

                    new_person_j = json.dumps(face_info, default=str)
                    print(face_info['name'])

                    db_helper.insert_new_event(camera_id, face_info['face_id']) # брать камеры из БД

                    qO.put(new_person_j)
                else:
                    print('pass')
            else:
                # записываем в БД новое лицо
                time_detection = datetime.datetime.now()
                if LAST_TIME_DETECTION_UNKNOWN:
                    delta_time_detection = time_detection - LAST_TIME_DETECTION_UNKNOWN
                    minutes_passed = delta_time_detection.seconds / 60
                else:
                    minutes_passed = TIME_DETECTION_UNKNOWN + 1

                print('UNKNOWN compare last detec: ' + str(minutes_passed))

                if minutes_passed > TIME_DETECTION_UNKNOWN:
                    relative_path = 'static/DataSet/unknowns/{0}.jpg'.format(str(time_detection))
                    path = directory + '/' + relative_path
                    cv2.imwrite(path, frame)
                    added_id = db_helper.insert_new_face(True, relative_path)
                    if added_id:
                        # записываем событие с неизвестным лицом
                        db_helper.insert_new_event(camera_id, added_id)
                    print('unknown  ' + str(added_id) + '    ' + relative_path)
                    LAST_TIME_DETECTION_UNKNOWN = time_detection

                    unknown_info = {}
                    unknown_info['datetime'] = time_detection
                    unknown_info['camera'] = camera_name
                    unknown_info['photo_path'] = relative_path
                    unknown_info['name'] = 'Unknown'
                    unknown_info['lastname'] = 'Unknown'
                    unknown_info['is_unknown'] = True

                    unknown_info_j = json.dumps(unknown_info, default=str)
                    qO.put(unknown_info_j)

        i += 1

        if cv2.waitKey(1) & 0xFF == ord('q'):
            break

queueImg = multiprocessing.Queue()
queueToSend = multiprocessing.Queue()

detector = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')
font = cv2.FONT_HERSHEY_SIMPLEX

# TODO: сделать получение камер из БД

cameras = db_helper.get_cameras()

# for c in cameras:
#     cam = cv2.VideoCapture(c['index'])
#     cam.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_HEIGHT)
#     cam.set(cv2.CAP_PROP_FRAME_WIDTH, FRAME_WIDTH)
#     cam.set(cv2.CAP_PROP_FPS, FRAME_FPS)
#     prc = multiprocessing.Process(target=CheckFace, args=(cam, queueImg, c, COUNT_FRAME_WITH_DATA,))
#     prc.start()
#     print(c['name'])
#     print(prc.pid)

# запуск камеры 2
# cam = {}
# cam['id'] = 2
# cam['name'] = 'Камера 2'
# entry_camera = cv2.VideoCapture(2)
# entry_camera.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_HEIGHT)
# entry_camera.set(cv2.CAP_PROP_FRAME_WIDTH, FRAME_WIDTH)
# entry_camera.set(cv2.CAP_PROP_FPS, FRAME_FPS)
# prc = multiprocessing.Process(target=CheckFace, args=(entry_camera, queueImg, cam, COUNT_FRAME_WITH_DATA,))
# prc.start()
# print('cam1')
# print(prc.pid)

# запуск камеры 1

# cam1 = {}
# cam1['id'] = 1
# cam1['name'] = 'Камера 1'
# exit_camera = cv2.VideoCapture(0)
# exit_camera.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_HEIGHT)
# exit_camera.set(cv2.CAP_PROP_FRAME_WIDTH, FRAME_WIDTH)
# exit_camera.set(cv2.CAP_PROP_FPS, FRAME_FPS)
# prc1 = multiprocessing.Process(target=CheckFace, args=(exit_camera, queueImg, cam1, COUNT_FRAME_WITH_DATA,))
# prc1.start()
# print('cam2')
# print(prc1.pid)

# запуск сервера
prc2 = multiprocessing.Process(target=StartWebServer, args=(queueToSend,))
prc2.start()
print(prc2.pid)

# FullRecognizedFace(queueImg, queueToSend)